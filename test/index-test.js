...
describe("Test the hello world skill", function() {
    ...
    it("Should launch the skill and get a response", function(done) {
        alexa.filter(function(request) {
            ...
            expect(result.response.outputSpeech.ssml).to.include("Hello! This is Cake walk. What is your birthday?");
            done();
        });
    });

    it("Should utter birthday date and get a response", function(done) {
        alexa.utter("I was born on February 23 1996").then((result) => {
            expect(result.response.outputSpeech.ssml).to.exist;
            expect(result.response.outputSpeech.ssml).to.include("I'll remember that you were born");
            done();
        });
    });
